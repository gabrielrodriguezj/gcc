...
static token_type_t check_keyword(const int start, const int length,
    const char* rest, const token_type_t type) {
...
static token_type_t identifierType() {
    switch (scanner.start[0]) {
        ...
        case 't': return check_keyword(2, 2, "ue", TOKEN_TRUE);
...
static token_t string() {
    ...
    if (is_at_end()) return errorToken("Unterminated string.");
